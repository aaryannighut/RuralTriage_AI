...
export function CheckSymptoms() {
  ...
  useEffect(() => {
    const fetchPatient = async () => {
      ...
      try {
        ...
        setError(err instanceof Error ? err.message : t("Failed to cross-reference patient profile."));
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleFollowUp = async () => {
    ...
    try {
        ...
        setError(t("Contextual analysis failed. Please try again."));
    } finally {
        setChatLoading(false);
    }
  };
  ...
  const handleAddSymptom = async () => {
    if (selectedSymptoms.length === 0 || !duration) {
      setError(t("MANDATORY FIELDS MISSING: Select at least one symptom and duration."));
      return;
    }
    ...
    try {
      ...
      if (patient && patient.id) {
        ...
        if (res.ok) {
         ...
         setSuccessMessage(t("INITIATING TRIAGE PROTOCOL."));
      }
      setTimeout(() => setSuccessMessage(""), 3000);
      callAIAnalysis(symptomNames, duration);
      setSelectedSymptoms([]); setDuration("");
    } catch (err) {
      setError(err instanceof Error ? err.message : t("TRANSMISSION ERROR"));
    } finally {
      setSaving(false);
    }
  };
  ...
  if (loading) return (
    <div className="p-12 text-center text-[#0056b3] uppercase font-bold flex items-center justify-center gap-3">
      <Loader2 className="w-5 h-5 animate-spin" /> {t("Retrieving Patient Payload...")}
    </div>
  );
  ...
  if (!user.userId) return (
    <div className="w-full max-w-4xl mx-auto p-6 mt-12 bg-red-50 border border-red-300 flex items-center flex-col text-center">
       ...
       <p className="text-red-800 font-semibold mt-2">{t("Authentication required for clinical symptom logging.")}</p>
    </div>
  );
  ...
  return (
    <div className="w-full space-y-6 text-slate-900 font-sans pb-12">
      ...
      <div className="border border-slate-300 bg-slate-50 p-6">
           <h1 className="text-2xl font-bold uppercase tracking-tight">{t("Symptom Triage System")}</h1>
           <p className="text-sm font-semibold text-slate-600 mt-1 uppercase tracking-wide">{t("Automated Symptom Analysis & Registry")}</p>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6 items-start">
        <div className="border border-slate-300 bg-white">
          <div className="bg-slate-100 border-b border-slate-300 p-4">
            <h2 className="font-bold uppercase tracking-wide text-sm">{t("1. Identify Current Symptoms")}</h2>
          </div>
          ...
          <div className="p-4 bg-slate-100 border-t border-b border-slate-300 mt-2">
            <h2 className="font-bold uppercase tracking-wide text-sm">{t("2. Duration of Symptoms")}</h2>
          </div>
          ...
          <div className="p-4 border-t border-slate-300">
             ...
             >
                {saving ? <Loader2 className="w-5 h-5 animate-spin"/> : t("Submit & Start Triage")}
             </button>
          </div>
        </div>
        ...
        <div className="border border-slate-300 bg-white h-[650px] flex flex-col relative overflow-hidden">
            <div className="bg-[#0056b3] text-white p-4 flex items-center justify-between sticky top-0 z-20">
              <div className="flex items-center gap-3">
                ...
                <h3 className="font-bold uppercase tracking-wider text-xs">{t("RuralTriage AI Engine")}</h3>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 bg-[#f5f9ff] space-y-6 flex flex-col">
              {!showResult ? (
                <div className="flex-1 flex flex-col items-center justify-center text-slate-300 text-center space-y-4">
                  ...
                  <p className="uppercase font-bold text-[10px] tracking-widest">{t("Awaiting Clinical Data...")}</p>
                </div>
              ) : (
                      ...
                      {event.role === "user" ? (
                        <div className="flex justify-end mb-4">
                           <div className="bg-[#0056b3] text-white p-3 border border-blue-800 shadow-sm max-w-[85%]">
                              <p className="text-xs font-bold leading-relaxed">{t(event.data as string)}</p>
                              <div className="text-[8px] font-black uppercase mt-1 opacity-50 text-right">{event.time} | {t("PATIENT QUERY")}</div>
                           </div>
                        </div>
                      ) : (
      ...
      {patient?.symptoms?.length ? (
        <div className="border border-slate-300 bg-white">
            <div className="bg-slate-100 border-b border-slate-300 p-4">
                <h3 className="font-bold uppercase tracking-wide text-xs">{t("Symptom Historical Log")}</h3>
            </div>
            <div className="max-h-[250px] overflow-y-auto divide-y divide-slate-300">
                ...
                <div key={symptom.id ?? idx} className="p-4 flex justify-between items-center hover:bg-slate-50">
                    <div>
                        <div className="font-bold text-xs uppercase text-slate-900">{t(symptom.symptom_name)}</div>
                        <div className="text-[10px] font-semibold text-[#0056b3] mt-1 uppercase tracking-wider">{t("Duration")}: {t(symptom.duration)}</div>
                    </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pb-4 border-l-4 border-slate-200 pl-4">
            ...
            <div className={`md:col-span-2 border-2 ${colors.border} ${colors.bg} p-5 shadow-md relative overflow-hidden`}>
                ...
                <div className="flex items-start gap-4">
                    ...
                    <div>
                        <h4 className="text-[10px] font-black text-slate-500 uppercase mb-1 tracking-widest">{t("Final Triage Decision")}</h4>
                        <h2 className={`text-2xl font-black uppercase tracking-tighter ${colors.text} leading-none mb-2`}>
                            {t(result.final_decision)}
                        </h2>
            ...
            <div className="md:col-span-2 border border-slate-200 bg-white p-3 shadow-sm">
                <h4 className="text-[8px] font-black text-slate-400 uppercase mb-1 tracking-widest">{t("Summary")}</h4>
                <p className="text-xs font-bold text-slate-900 leading-tight">{t(result.summary)}</p>
            </div>
            ...
            <div className={`border p-3 bg-white shadow-sm ${result.risk_level === "Critical" ? "border-red-600" : result.risk_level === "High" ? "border-red-400" : result.risk_level === "Medium" ? "border-yellow-400" : "border-green-400"}`}>
                <h4 className="text-[8px] font-black text-slate-400 uppercase mb-1 tracking-widest">{t("Baseline Risk")}</h4>
            ...
            <div className="border border-slate-200 bg-white p-3 shadow-sm">
                <h4 className="text-[8px] font-black text-slate-400 uppercase mb-2 tracking-widest">{t("Possible Conditions")}</h4>
                <div className="flex flex-wrap gap-1">
                    {result.possible_conditions.map((c, i) => (
                        <span key={i} className="text-[9px] font-bold bg-slate-50 text-slate-600 px-2 py-0.5 border border-slate-200">{t(c)}</span>
                    ))}
                </div>
            </div>

            <div className="md:col-span-2 border border-slate-200 bg-white p-3 shadow-sm">
                <h4 className="text-[8px] font-black text-slate-400 uppercase mb-2 tracking-widest">{t("Clinical Explanation")}</h4>
                <ul className="space-y-1">
                    {result.explanation.map((e, i) => (
                        <li key={i} className="text-[10px] font-bold text-slate-600 flex items-start gap-2">
                           <CheckCircle className="w-3 h-3 text-green-600 mt-0.5" /> {t(e)}
                        </li>
                    ))}
                </ul>
            </div>
            ...
            <div className="border border-slate-200 bg-red-50 p-3 shadow-sm">
                <h4 className="text-[8px] font-black text-red-400 uppercase mb-1 tracking-widest">{t("Medical Advice")}</h4>
                {result.medical_advice.map((a, i) => (
                    <p key={i} className="text-[10px] font-black text-red-800 leading-tight uppercase mb-1 italic">! {t(a)}</p>
                ))}
            </div>

            <div className="border border-slate-200 bg-white p-3 shadow-sm">
                <h4 className="text-[8px] font-black text-slate-400 uppercase mb-2 tracking-widest">{t("Actionable Steps")}</h4>
                <ul className="space-y-1">
                    {result.next_steps.map((s, i) => (
                        <li key={i} className="text-[10px] font-bold text-[#0056b3] flex items-center gap-2">
                            <Clock className="w-3 h-3" /> {t(s)}
                        </li>
                    ))}
                </ul>
            </div>
            ...
            <div className="md:col-span-2 border border-slate-300 bg-[#eff6ff] p-3 shadow-sm">
                <h4 className="text-[8px] font-black text-slate-700 uppercase mb-2 tracking-widest">{t("Precautions (2-3 Days)")}</h4>
            ...
            <div className="md:col-span-2 flex items-center gap-4 opacity-20 mt-2">
                ...
                <div className="text-[8px] font-black uppercase whitespace-nowrap tracking-[0.2em]">{time} | {t("AI ANALYSIS")}</div>
                <div className="flex-1 h-[1px] bg-slate-400"></div>
            </div>
        </div>
    );

      ...
      <div className="max-w-6xl mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-12 mb-10">
        ...
        <div className="space-y-4 text-center md:text-left">
          <Link to="/" className="flex items-center justify-center md:justify-start gap-2">
            ...
            <span className="text-xl font-bold tracking-tight text-white">{t("RuralTriage AI")}</span>
          </Link>
          <p className="text-sm text-slate-300 font-medium leading-relaxed max-w-xs mx-auto md:mx-0">
            {t("Empowering rural communities with AI-driven diagnostic triage and seamless telemedicine access.")}
          </p>
        </div>

        {/* Section B: Quick Links */}
        <div className="space-y-4 text-center">
          <h3 className="text-xs font-black uppercase tracking-[0.2em] text-[#00aaff]">{t("Quick Navigation")}</h3>
          <div className="grid grid-cols-2 gap-2 text-sm font-bold">
            <Link to="/" className="hover:text-[#00aaff] transition-colors">{t("Dashboard")}</Link>
            <Link to="/talk-to-doctor" className="hover:text-[#00aaff] transition-colors">{t("Talk to Doctor")}</Link>
            <Link to="/check-symptoms" className="hover:text-[#00aaff] transition-colors">{t("Check Symptoms")}</Link>
            <Link to="/find-medicines" className="hover:text-[#00aaff] transition-colors">{t("Find Medicines")}</Link>
            <Link to="/health-records" className="hover:text-[#00aaff] transition-colors">{t("Health Records")}</Link>
            <Link to="/support" className="hover:text-[#00aaff] transition-colors flex items-center justify-center gap-1 group">
               {t("Support")} <ExternalLink className="w-3 h-3 text-slate-500 group-hover:text-[#00aaff]" />
            </Link>
          </div>
        </div>
        ...
        <div className="space-y-4 text-center md:text-right">
          <h3 className="text-xs font-black uppercase tracking-[0.2em] text-[#00aaff]">{t("Contact Registry")}</h3>
          <div className="space-y-3 flex flex-col items-center md:items-end">
            ...
            <div className="flex items-center gap-2 group cursor-pointer">
              <span className="text-sm font-bold text-slate-200 group-hover:text-white">{t("India")}</span>
      ...
      <div className="max-w-6xl mx-auto px-6 border-t border-slate-800/50 pt-8 mt-4 flex flex-col md:flex-row justify-between items-center gap-4 text-center md:text-left">
        <div className="space-y-1">
          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">
            © {currentYear} {t("RuralTriage AI • All Rights Reserved")}
          </p>
          <div className="flex items-center gap-2 justify-center md:justify-start">
            ...
            <p className="text-[9px] font-black text-slate-600 uppercase tracking-tighter italic">
              {t("Official Medical Triage Deployment Portal")}
            </p>
          </div>
        </div>
        
        <div className="text-[10px] font-bold text-slate-500 space-x-4 uppercase tracking-[0.1em]">
          <Link to="/privacy" className="hover:text-white transition-colors cursor-pointer">{t("Privacy Policy")}</Link>
          <span className="hover:text-white transition-colors cursor-pointer cursor-not-allowed opacity-40">{t("Terms of Service")}</span>
          <span className="text-[#00aaff]/60 px-3 py-1 bg-blue-900/30 rounded-full border border-blue-800/20">{t("Educational Demo")}</span>
        </div>
      </div>
    </footer>
  );
